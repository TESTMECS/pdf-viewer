...
import os
import logging
from database import db, init_db, Tag
...
logger = logging.getLogger("PDF VIEWER")

# Get configuration from environment variables or use defaults
BOOKS_DIR = os.environ.get(
    "PDF_DIR",
    os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "books"
    ),  # Changed to "Books" for Calibre library symlink
)
DB_PATH = os.environ.get(
    "DATABASE_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "books.db"),
)
...
def get_books_structure():
    """Scan the books directory and return a dictionary of folders and their book files."""
    structure = {}
    # Define supported file extensions (add or remove as needed)
    try:
        # Set followlinks=True to follow symbolic links, but be careful of circular references
        for root, _, files in os.walk(BOOKS_DIR, followlinks=True):
            ...
            for f in files:
                ...
                if ext in SUPPORTED_EXTENSIONS:
                    book_files.append(f)
            if book_files:  # Only add folders that contain books
                structure[folder] = book_files
...
def index():
    ...
    return render_template(
        "index.html", books=books, book_tags=book_tags, available_tags=available_tags
    )


@app.route("/books/<path:filepath>")
def serve_pdf(filepath):
    """Serve book files from the books directory."""
    directory = os.path.dirname(filepath)
    filename = os.path.basename(filepath)

    if directory:
        book_path = os.path.join(BOOKS_DIR, directory)
    else:
        book_path = BOOKS_DIR
    ...
    return send_from_directory(
        book_path, filename, mimetype=mime_types.get(ext, "application/octet-stream")
    )
            ...
            if tag.name in AVAILABLE_TAGS and name != tag.name:
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Cannot change name of default tags",
                        }
                    ),
                    400,
                )
    ...
    try:
        with app.app_context():
            ...
            if tag.name in AVAILABLE_TAGS:
                return (
                    jsonify({"success": False, "error": "Cannot delete default tags"}),
                    400,
                )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(BOOKS_DIR):
        logger.warning(
            f"Books directory not found at {BOOKS_DIR}. Creating empty directory."
        )
